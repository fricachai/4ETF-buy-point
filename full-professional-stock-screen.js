...
function technicalFromCloses(rawCloses) {
  const closes = rawCloses.filter(Number.isFinite);
...
function scoreRecords(records) {
  const fields = ['revenueYoy', 'revenueYtdYoy', 'operatingMargin', 'debtRatio', 'currentRatio', 'earningsYield', 'pe', 'pb', 'yield',
    'etfFlow5', 'etfFlow10', 'etfFlow20', 'activeFlow5', 'institutionValue5', 'foreignNetValue5',
  ...
  for (const record of records) {
    ...
    const ownership = WEIGHTS.ownership * (
      0.18 * p(record, 'etfFlow5', true) +
      0.12 * p(record, 'etfFlow10', true) +
      0.07 * p(record, 'etfFlow20', true) +
      0.05 * acceleration +
      0.14 * p(record, 'activeFlow5', true) +
      0.08 * breadthScore +
      0.08 * p(record, 'institutionValue5', true) +
      0.10 * p(record, 'foreignNetValue5', true) +
    ...
    const technical = WEIGHTS.technical * (
      0.42 * trendScore + 0.24 * idealRsiScore(t?.rsi14) + 0.14 * macdScore + 0.20 * extensionScore
    );
    ...
    const catalyst = WEIGHTS.catalyst * (0.35 * revenueAcceleration + 0.25 * breadthScore + 0.25 * catalystEvent + 0.15 * growthConsistency);
...
function classifyRecord(record) {
  ...
  if (m.institution5 < 0 && m.institution10 < 0) reasons.push('三大法人5/10日同步賣超');
  ...
  const timingReject = reasons.some(reason => /改善不足|調節|1\/3日|主動ETF|法人5\/10日|外資持股|尚未上彎|未守20日|過熱|高波動|急跌反轉|除權息|共識不足|估值高/.test(reason));
  ...
  if (hardReject) {
    ...
    action = '等回測或確認';
    bucket = 'B';
  } else if (record.adjustedScore >= 67 && analysisPrice >= t?.ema20 && t?.ma20Slope5 > 0 && t?.ema60Slope >= 0 && record.etf.flowPct[5] > 0 && record.etf.flowPct[10] >= 0 && record.etf.activeChanges[5] >= 0 && foreignAligned) {
    action = '可分批布局';
    bucket = 'A';
...
function thesisText(record) {
  ...
  if (m.institution5 > 0) positives.push(`三大法人5日合計 ${signed(m.institution5, 0, ' 張')}`);
...
function selectTopThree(records) {
  ...
  if (selected.length < 3) {
    const watchlist = records.filter(record => record.bucket === 'B' && record.signalRobust && record.adjustedScore >= 65)
      .sort((a, b) => {
        const penalty = record => sum(record.rejectionReasons.map(reason => /2日內除權息/.test(reason) ? 0.5 : (/尚未上彎|法人5\/10日/.test(reason) ? 2 : 3)));
...
function recordForOutput(record, rank) {
  return {
    ...
    institution: { d1: record.metrics.institution1, d5: record.metrics.institution5, d10: record.metrics.institution10 },
    foreign: {
    ...
    technical: Object.fromEntries(Object.entries(record.technical || {}).map(([key, value]) => [key, round(value, 2)])),
...
function buildCsv(rows) {
  const columns = [
    ['排名', 'rank'], ['代號', 'code'], ['名稱', 'name'], ['市場', 'market'], ['產業', 'industry'], ['行動', 'action'],
    ...
    ['法人1日張', 'institution.d1'], ['法人5日張', 'institution.d5'], ['法人10日張', 'institution.d10'],
    ...
    ['外資買賣超1日張', 'foreign.netBuy1'], ['外資買賣超5日張', 'foreign.netBuy5'], ['外資買賣超10日張', 'foreign.netBuy10'],
    ['RSI14', 'technical.rsi14'], ['距20EMA%', 'technical.distanceEma20'], ['5日報酬%', 'technical.return5'], ['淘汰原因', 'rejectionReasons']
  ];
...
function buildHtml(report) {
  const topCards = report.topThree.map((row, index) => `
    <article class="pick">
      <div class="pick-head"><span class="rank">${index + 1}</span><div><h3>${escapeHtml(row.code)} ${escapeHtml(row.name)}</h3><p>${escapeHtml(row.industry)} · ${escapeHtml(row.action)}</p></div><strong>${fmt(row.score)}</strong></div>
      <div class="metrics"><span>即時價<b>${fmt(row.livePrice ?? row.close, 2)}</b></span><span>信心度<b>${fmt(row.confidence, 0)}%</b></span><span>本益比<b>${fmt(row.valuation.pe)}</b></span><span>RSI<b>${fmt(row.technical.rsi14)}</b></span></div>
      <dl><dt>值得布局的理由</dt><dd>${escapeHtml(row.thesis)}</dd><dt>市場可能已經反映的部分</dt><dd>${escapeHtml(row.pricedIn)}</dd><dt>市場可能低估的地方</dt><dd>${escapeHtml(row.possibleUnderestimate)}</dd><dt>最先要注意的風險</dt><dd>${escapeHtml(row.rejectionReasons[0] || '目前未觸發硬性風險，但仍不適合一次買足。')}</dd><dt>建議怎麼布局</dt><dd>${escapeHtml(row.entryPlan)}</dd><dt>減碼或停止買進的警訊</dt><dd>${escapeHtml(row.riskAction)}</dd></dl>
      <div class="flow"><span>ETF 5日 ${signed(row.etf.d5, 0, '張')}</span><span>主動ETF 5日 ${signed(row.etf.activeD5, 0, '張')}</span><span>外資持股5日 ${signed(row.foreign.holdingD5, 0, '張')}</span><span>外資買賣超5日 ${signed(row.foreign.netBuy5, 0, '張')}</span></div>
    </article>`).join('');

  const topRows = report.ranking.slice(0, 30).map(row => `
    <tr><td>${row.rank}</td><td><b>${escapeHtml(row.code)}</b> ${escapeHtml(row.name)}</td><td>${escapeHtml(row.action)}</td><td>${fmt(row.score)}</td><td>${fmt(row.confidence, 0)}%</td><td>${signed(row.fundamentals.revenueYoy, 1, '%')}</td><td>${fmt(row.valuation.pe)}</td><td>${signed(row.etf.d5, 0)}</td><td>${signed(row.etf.activeD5, 0)}</td><td>${signed(row.foreign.holdingD5, 0)}</td><td>${signed(row.foreign.holdingD10, 0)}</td><td>${signed(row.foreign.netBuy5, 0)}</td><td>${fmt(row.technical.rsi14)}</td><td>${escapeHtml(row.rejectionReasons[0] || '—')}</td></tr>`).join('');
...
<section class="section"><h2>前30名與主要風險</h2><p class="section-lead">外資持股增減是存量，外資買賣超是流量；兩者與ETF方向一致時，才提高布局優先度。</p><div class="table-wrap"><table><thead><tr><th>#</th><th>股票</th><th>行動</th><th>總分</th><th>信心</th><th>營收YoY</th><th>PE</th><th>ETF 5日</th><th>主動ETF 5日</th><th>外資持股5日</th><th>外資持股10日</th><th>外資買賣超5日</th><th>RSI</th><th>最先注意風險</th></tr></thead><tbody>${topRows}</tbody></table></div></section>

<section class="section"><h2>完整 ${report.meta.stockCount} 檔上市股票排名</h2><div class="filters"><input id="search" type="search" placeholder="搜尋代號或名稱"><select id="bucket"><option value="">全部行動</option><option value="A">A 可分批布局</option><option value="B">B 觀察／等回測</option><option value="C">C 等待轉強</option><option value="D">D 排除</option></select><select id="sector"><option value="">全部產業</option>${[...new Set(report.ranking.map(row => row.sector))].sort().map(value => `<option>${escapeHtml(value)}</option>`).join('')}</select></div><div class="table-wrap"><table><thead><tr><th>#</th><th>股票</th><th>產業</th><th>行動</th><th>分數</th><th>信心</th><th>即時/收盤</th><th>營收YoY</th><th>累計YoY</th><th>PE</th><th>ETF 5日</th><th>ETF 10日</th><th>主動5日</th><th>外資持股比</th><th>外資持股5日</th><th>外資持股10日</th><th>外資買賣超5日</th><th>RSI</th><th>距20EMA</th><th>主要風險</th></tr></thead><tbody id="fullRows"></tbody></table></div><p class="small">可用搜尋與下拉選單縮小範圍；外資持股比率高低本身不是買賣訊號，重點是持股變化、買賣超與ETF方向是否一致。</p></section>

<section class="section method"><article><h3>專業篩選架構</h3><div class="weights"><span><b>30%</b>基本面與成長</span><span><b>15%</b>估值</span><span><b>20%</b>ETF／外資／法人籌碼</span><span><b>15%</b>技術與葛蘭碧時機</span><span><b>10%</b>催化劑</span><span><b>10%</b>風險與流動性</span></div><p>ETF採20／10／5／3／1日多時間窗，主動與被動ETF分開。外資同時檢查官方持股存量1／5／10／20日變化，以及外資買賣超流量；兩者方向不一致時降低評級，不把單日買超誤認為長期加碼。技術面使用5、20、60日EMA、RSI、MACD、乖離與趨勢斜率，並對當日除權息做比較價格調整。</p></article><article><h3>硬性與時機檢查</h3><ul><li>只保留上市股票，排除上櫃。</li><li>資料信心度低於65%或成交金額不足者排除。</li><li>營收同步衰退或中期趨勢向下者排除。</li><li>ETF 5／10日同步減碼者等待轉強。</li><li>外資持股5／10日同步下降且外資賣超者，不列立即布局。</li><li>外資持股期間出現非市場異動時，該段趨勢不直接計分。</li><li>處置、過熱、估值與除息事件改列等待。</li></ul></article></section>

<section class="section"><h2>證據與限制</h2><p class="section-lead">證交所說明，外資持股變化除集中市場買賣外，也可能受借券、海外存託憑證、股本異動、ETF申購買回與國籍變更影響。因此本系統把外資持股與買賣超分開，不把兩者當成完全相同。本報告仍未取得券商一致預估、目標價、完整自由現金流與個人持倉成本，候選名單不等於無條件買進。</p><ul class="sources">${sourceRows}</ul></section>
</main>
<script>
const rows=${dataJson};const body=document.getElementById('fullRows');const search=document.getElementById('search');const bucket=document.getElementById('bucket');const sector=document.getElementById('sector');
...
function draw(){const q=search.value.trim().toLowerCase();const filtered=rows.filter(r=>(!q||(r.code+' '+r.name).toLowerCase().includes(q))&&(!bucket.value||r.bucket===bucket.value)&&(!sector.value||r.sector===sector.value));body.innerHTML=filtered.map(r=>'<tr><td>'+r.rank+'</td><td><b>'+e(r.code)+'</b> '+e(r.name)+'</td><td>'+e(r.industry)+'</td><td class="tag-'+r.bucket+'">'+e(r.action)+'</td><td>'+n(r.score)+'</td><td>'+n(r.confidence,0)+'%</td><td>'+n(r.livePrice??r.close,2)+'</td><td>'+s(r.fundamentals.revenueYoy,1,'%')+'</td><td>'+s(r.fundamentals.revenueYtdYoy,1,'%')+'</td><td>'+n(r.valuation.pe)+'</td><td>'+s(r.etf.d5)+'</td><td>'+s(r.etf.d10)+'</td><td>'+s(r.etf.activeD5)+'</td><td>'+n(r.foreign.holdingRatio,2)+'%</td><td>'+s(r.foreign.holdingD5)+'</td><td>'+s(r.foreign.holdingD10)+'</td><td>'+s(r.foreign.netBuy5)+'</td><td>'+n(r.technical.rsi14)+'</td><td>'+s(r.technical.distanceEma20,1,'%')+'</td><td>'+e((r.rejectionReasons||[]).join('；')||'—')+'</td></tr>').join('')}
[search,bucket,sector].forEach(el=>el.addEventListener('input',draw));draw();
</script></body></html>`;
}
...
async function main() {
  ...
  const instDates = (institution.dates || []).slice(0, 10);
  ...
  const institutional = code => {
    ...
    return {
      ...
      total5: get(3, 5), total10: get(3, 10), foreign5: get(0, 5), foreign10: get(0, 10), trust5: get(1, 5)
    };
  };
  ...
  const records = stockEntries.map(([code, stock], index) => {
    ...
    const technical = technicalFromCloses(closes);
    ...
    return {
      ...
      metrics: {
        ...
        institution10: inst.total10, institutionValue5: inst.total5 * (price || 0) * 1000,
        foreignNet1: inst.foreign1, foreignNet5: inst.foreign5, foreignNet10: inst.foreign10,
  ...
  const report = {
    ...
    methodology: {
      weights: WEIGHTS,
      hierarchy: '只看上市股票；ETF以20日背景、10日延續、5日反轉警報；外資同時看持股存量1/5/10/20日與買賣超流量',
      hardFilters: ['信心度>=65%', '單日成交金額>=5,000萬元', '營收未同步明顯衰退', '中期趨勢未明顯向下', 'ETF 5/10日未同步減碼', '外資持股5/10日與外資買賣超未同步轉弱', '最終布局需具ETF加碼經濟金額或跨基金共識', '未處於處置風險'],
    ...
    sources: {
      ...
      '籌碼小宇 法人逐日資料': SOURCES.institution,
